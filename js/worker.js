...
function processFirePoints(fireFeatures, boundary) {
	const points = fireFeatures.features
		.map((f) => {
	...
	return {
		points,
		areas: calculateBurntAreas(points)
	};
}
...
function calculateBurntAreas(points) {
	...
	const clusters = clustered.features.reduce((acc, feature) => {
		...
		if (id !== undefined) (acc[id] = acc[id] || []).push(feature);
		return acc;
	}, {});

	const areaPolygons = Object.values(clusters)
		.map((clusterPoints) => {
			const buffers = clusterPoints.map((point) =>
				turf.buffer(point, 1, {
					units: 'kilometers'
				})
			);
			return turf.buffer(
				turf.buffer(turf.union(turf.featureCollection(buffers)), 1, {
					units: 'kilometers'
				}),
				-1,
				{
...
async function handleInitData({ url }) {
	self.postMessage({
		type: 'progress',
...
async function handleSatelliteData({ dayRange }) {
	self.postMessage({
		type: 'progress',
	...
	if (!workerPortugalGeometry)
		return self.postMessage({
			type: 'error',
			message: 'Geometria de Portugal não carregada.'
		});
	...
	for (const [key, url] of Object.entries(urls)) {
		self.postMessage({
			type: 'progress',
			message: `A obter dados ${key.toUpperCase()}...`
		});
	...
	if (!results.modis && !results.viirs)
		self.postMessage({
			type: 'error',
			message: 'Nenhum dado de satélite recente encontrado.'
		});
	else
		self.postMessage({
...
async function handleRiskData() {
	...
	if (!workerConcelhosGeoJSON)
		return self.postMessage({
			type: 'error',
			message: 'GeoJSON dos Concelhos não carregado.'
		});
		...
		if (data) {
			...
			riskLayers[`Risco ${date}`] = {
				...
				features: workerConcelhosGeoJSON.features.map((f) => ({
					...
					properties: {
						...f.properties,
						rcm: data.local[f.properties.DICO]?.data?.rcm,
						fillColor: getRiskColor(data.local[f.properties.DICO]?.data?.rcm)
					}
				}))
			};
		}
...
async function handleFiresData() {
	...
	if (!data?.features)
		return self.postMessage({
			type: 'error',
			message: `Falha na obtenção de dados de incêndios.`
		});
	...
	const processedFires = data.features.map(({ properties: p }) => ({
		...
		lat: p.Latitude,
		lng: p.Longitude,
		statusCode: statusMap[p.EstadoOcorrencia] || 0,
		man: p.Operacionais,
...
self.onmessage = async ({ data }) => {
	try {
		...
		self.postMessage({
			type: 'error',
			message: `Ocorreu um erro no worker: ${err.message}`
		});
	}
};