class SPA {
	...
	julian_ephemeris_day(jd, delta_t) {
	...
	julian_ephemeris_millennium(jce) {
		return (jce / 10);
	}
	...
	mean_elongation_moon_sun(jce) {
		return this.third_order_polynomial(1 / 189474, -0.0019142, 445267.11148, 297.85036, jce);
	}

	mean_anomaly_sun(jce) {
		return this.third_order_polynomial(-1 / 300000, -0.0001603, 35999.05034, 357.52772, jce);
	}

	mean_anomaly_moon(jce) {
		return this.third_order_polynomial(1 / 56250, 0.0086972, 477198.867398, 134.96298, jce);
	}

	argument_latitude_moon(jce) {
		return this.third_order_polynomial(1 / 327270, -0.0036825, 483202.017538, 93.27191, jce);
	}

	ascending_longitude_moon(jce) {
		return this.third_order_polynomial(1 / 450000, 0.0020708, -1934.136261, 125.04452, jce);
	}
	...
	topocentric_right_ascension(alpha_deg, delta_alpha) {
	...
	topocentric_elevation_angle_corrected(e0, delta_e) {
		return e0 + delta_e;
	}
	...
	topocentric_azimuth_angle(azimuth_astro) {
		return this.limit_degrees(azimuth_astro + 180);
	}
	...
	validate_inputs(spa) {
		...
		if (spa.hour === 24 && (spa.minute > 0 || spa.second > 0)) {
			return 4;
		}
		...
		if ((spa.function === this.SPA_ZA_INC || spa.function === this.SPA_ALL) && (Math.abs(spa.slope) > 360 || Math.abs(spa.azm_rotation) > 360)) {
			return 14;
		}
		return 0;
	}
	...
	calculate_eot_and_sun_rise_transit_set(spa) {
		...
		if (h0 >= 0) {
			...
			for (let i = 0; i < 3; i++) {
				...
				let n = m_rts[i] + spa.delta_t / 86400;
	...
	greenwich_mean_sidereal_time(jd, jc) {
		return this.limit_degrees(280.46061837 + 360.98564736629 * (jd - 2451545) + jc * jc * (0.000387933 - jc / 38710000));
	}
	...
	eot(m, alpha, del_psi, epsilon) {
		return this.limit_minutes(4 * (m - 0.0057183 - alpha + del_psi * Math.cos(this.deg2rad(epsilon))));
	}
	...
	approx_sun_rise_and_set(m_transit, h0) {
		...
		return {
			sunrise: this.limit_zero2one(m_transit - h0_dfrac), sunset: this.limit_zero2one(m_transit + h0_dfrac), suntransit: this.limit_zero2one(m_transit)
		};
	}
	...
	rts_sun_altitude(latitude, delta_prime, h_prime) {
		return this.rad2deg(Math.asin(Math.sin(this.deg2rad(latitude)) * Math.sin(this.deg2rad(delta_prime)) + Math.cos(this.deg2rad(latitude)) * Math.cos(this.deg2rad(delta_prime)) * Math.cos(this.deg2rad(h_prime))));
	}

	sun_rise_and_set(m_rts, h_rts, delta_prime, latitude, h_prime, h0_prime, sun) {
		return m_rts[sun] + (h_rts[sun] - h0_prime) / (360 * Math.cos(this.deg2rad(delta_prime[sun])) * Math.cos(this.deg2rad(latitude)) * Math.sin(this.deg2rad(h_prime[sun])));
	}

	dayfrac_to_local_hr(dayfrac, timezone) {
		return 24 * this.limit_zero2one(dayfrac + timezone / 24);
	}
}