((window, document) => {
	...
	const baseLayersConfig = {
		...
		'Satellite - Dawn': {
			layer: 'mapbox://styles/mapbox/standard-satellite',
			style: 'dawn'
		},
		'Satellite - Day': {
			layer: 'mapbox://styles/mapbox/standard-satellite',
		...
		currentWorker.onmessage = (e) => {
			...
			if (type === 'progress') {
				...
				overlayLayers['MODIS'].hotspotData = {
					type: 'FeatureCollection',
					features: data.modis?.points || []
				};
				overlayLayers['MODIS'].areaData = {
					type: 'FeatureCollection',
					features: data.modis?.areas?.features || []
				};
				overlayLayers['VIIRS'].hotspotData = {
					type: 'FeatureCollection',
					features: data.viirs?.points || []
				};
				overlayLayers['VIIRS'].areaData = {
					type: 'FeatureCollection',
					features: data.viirs?.areas?.features || []
				};
